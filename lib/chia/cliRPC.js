  ...
  _createPlot(args) {
    ...
    let subProc = cp.exec(
      ...
      (err) => {
        ...
        let result = { ...out, endTime: Date.now(),error:(err!=null)?err.toString():null }
        fs.writeFileSync(path.join(jobPath,"job.on.create.json"),JSON.stringify(result,4,4))
    ...
    fs.writeFileSync(path.join(jobPath,"job.on.create.json"),JSON.stringify(out,4,4))
    return out;
  }

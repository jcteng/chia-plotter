    ...
    let res = await new Promise(async (r) => {
      ...
      for await (let line of rl) {
        ...
        if (count++ == 0) {
          ...
          this.emit("sample", Object.values(groupSample));
        }
      }
    });
...
async function main() {
  ...
  instance.on("sample", s=>console.table(s.map(i=>({
    ppid2pid:`${i.CreatingProcessID}->${i.pid}`,
    name:i.name,
    mem: humanize.filesize(i.WorkingSet),
    mem_max:humanize.filesize(i.WorkingSetPeak),
    read:humanize.filesize(i.IOReadBytessec),
    write:humanize.filesize(i.IOWriteBytessec),
    cpu: i.ProcessorTime.toFixed(2)
  }))));
